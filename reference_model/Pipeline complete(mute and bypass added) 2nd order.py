...
# Pipeline: Mic (I2S) → Gain → 3-Band IIR EQ → Delay → Limiter → Mute → Speaker (I2S)
    ...
    for bs in range(0, N, BLOCK_SIZE):
        ...
        for n in range(bs, be):
            ...
            if bypass_eq:
                ...
                x_eq=g_low*lp_y+g_mid*bp_y+g_high*hp_y
            out_eq[n]=x_eq
...
# ── Save Output ───────────────────────────────────────────────────────────────
wavfile.write("pipeline_complete_including_mute_bypass_2nd_order.wav", fs, np.int16(stage_limiter*32767))
print(f"\n  Saved: pipeline_complete_including_mute_bypass_2nd_order.wav")
