...
# Pipeline: Mic (I2S) → Gain → 3-Band IIR EQ (4th Order) → Delay → Limiter → Mute → Speaker (I2S)
# Added    : Mute Gate + Per-Block Bypass flags + Float Mute
...
GATE_THRESHOLD    = 0.00001   # −100 dBFS — blocks at or below human hearing threshold

# ── Block-Based Pipeline — 4th Order EQ + Bypass + Mute Gate + Float Mute ────
...
def run_pipeline(signal, fs, BLOCK_SIZE, gain,
                 ...
                 bypass_gain=False, bypass_eq=False,
                 bypass_delay=False, bypass_limiter=False,
                 mute=0.0):
    ...
    for bs in range(0, N, BLOCK_SIZE):
        ...
        # ── Mute Gate — checked once before Gain ──────────────────────────────
        # Silent block → input copied directly to output, pipeline skipped
        # Threshold: −100 dBFS (0.00001) — human hearing lower limit
        block_energy = np.max(np.abs(signal[bs:be]))
        if block_energy <= GATE_THRESHOLD:
            output[bs:be]    = signal[bs:be]   # input copied directly to output
            out_gain[bs:be]  = signal[bs:be]
            out_eq[bs:be]    = signal[bs:be]
            out_delay[bs:be] = signal[bs:be]
            gated_blocks    += 1
            print(f"  [MUTE GATE] Block {bs//BLOCK_SIZE:>4} "
                  f"@ sample {bs:>6} — silent input, copied to output")
            continue
        ...
        for n in range(bs, be):
            ...
            # ── EQ — 4th Order (Section 1 → Section 2 per band) ──────────────
            if bypass_eq:
                x_eq = x_g
            else:
                # Bass — LP
            ...
            # ── Float Mute — applied after Limiter ────────────────────────────
            # mute=0.0 → output unchanged  (button not pressed)
            # mute=1.0 → output = 0.0      (button pressed — speaker silenced)
            # Mirrors physical GPIO mute button on ESP32 hardware
            output[n] = output[n] * (1.0 - mute)

    return output, out_gain, out_eq, out_delay, gated_blocks
...
# ── Bypass Configuration — change these to test ───────────────────────────────
bypass_gain    = False   # True  = skip gain block
bypass_eq      = False   # True  = skip EQ block
bypass_delay   = False   # True  = skip delay block
bypass_limiter = False   # True  = skip limiter block
mute           = 0.0     # 0.0   = unmuted (normal), 1.0 = muted (speaker silent)

# ── Run Pipeline ──────────────────────────────────────────────────────────────
stage_limiter, stage_gain, stage_eq, stage_delay, gated_blocks = run_pipeline(
    ...
    eq_preset,
    delay_samples, feedback, mix, threshold,
    bypass_gain=bypass_gain, bypass_eq=bypass_eq,
    bypass_delay=bypass_delay, bypass_limiter=bypass_limiter,
    mute=mute
)
clipped      = np.sum(np.abs(stage_delay) > threshold)
total_blocks = int(np.ceil(len(signal) / BLOCK_SIZE))
...
print("─" * 55)
print(f"  Bypass Gain    : {'ON  ← gain skipped'    if bypass_gain    else 'OFF'}")
print(f"  Bypass EQ      : {'ON  ← EQ skipped'      if bypass_eq      else 'OFF'}")
print(f"  Bypass Delay   : {'ON  ← delay skipped'   if bypass_delay   else 'OFF'}")
print(f"  Bypass Limiter : {'ON  ← limiter skipped' if bypass_limiter else 'OFF'}")
print(f"  Mute           : {'ON  ← speaker silenced' if mute == 1.0   else 'OFF'}")
print(f"  Gate Threshold : {GATE_THRESHOLD}  (−100 dBFS)")
print("─" * 55)
print(f"  Input  peak   : {np.max(np.abs(signal)):.4f}")
print(f"  After Gain    : {np.max(np.abs(stage_gain)):.4f}  {'(bypassed)' if bypass_gain else ''}")
print(f"  After EQ      : {np.max(np.abs(stage_eq)):.4f}  {'(bypassed)' if bypass_eq else ''}")
print(f"  After Delay   : {np.max(np.abs(stage_delay)):.4f}  {'(bypassed)' if bypass_delay else ''}")
print(f"  After Limiter : {np.max(np.abs(stage_limiter)):.4f}  {'(bypassed)' if bypass_limiter else '✔'}")
print(f"  Clipped       : {clipped} samples ({clipped/len(signal)*100:.2f}%)")
print("─" * 55)
if gated_blocks == total_blocks:
    print(f"  ⚠  Entire input is silent — input copied to output directly")
    print(f"     All {total_blocks} blocks passed through without processing")
elif gated_blocks > 0:
    print(f"  ⚠  {gated_blocks} / {total_blocks} blocks were silent — input copied to output")
else:
    print(f"  ✔  No silent blocks — full audio processed ({total_blocks} blocks)")

# ── Save Output ───────────────────────────────────────────────────────────────
wavfile.write("pipeline_complete_including_mute_bypass_4th_order.wav", fs, np.int16(stage_limiter*32767))
print(f"\n  Saved: pipeline_complete_including_mute_bypass_4th_order.wav")
