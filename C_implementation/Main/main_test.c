#include <stdio.h>
// ADDED: libsndfile header for WAV file handling 
#include <sndfile.h>
#include <stdbool.h>
...
int main(void)
{
    ...
    // FILE OPEN ADDITIONS
    // Replaced standard FILE structures with libsndfile structures
    SF_INFO sfinfo = {0};
    ...
    // Type changed to sf_count_t for audio element stream tracking 
    sf_count_t read_count;

    // PROCESS LOOP ADDITIONS  
    // CHANGED: Swapped fread with sf_read_float for proper WAV parsing
    while ((read_count = sf_read_float(infile, buffer, BLOCK_SIZE)) > 0)
    {
        if (enable_bypass)
        {
        ...
        // CHANGED: Swapped fwrite with sf_write_float to structure output WAV frames
        sf_write_float(outfile,
                       final_out,
                       read_count);
    }

    // DELAY TAIL ADDITIONS
    ...
    if (enable_delay)
    {
        ...
        while (remain > 0)
        {
            ...
            // CHANGED: Replaced fwrite with sf_write_float for delay tail
            sf_write_float(outfile,
                           final_out,
                           block);
    ...
    // CHANGED: Swapped fclose with sf_close for proper metadata flushing
    sf_close(infile);
    sf_close(outfile);
